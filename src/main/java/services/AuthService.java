...
public class AuthService {
    ...
    public User register(String email, String password, String name) throws Exception {
        System.out.println("DEBUG AuthService: Starting registration for " + email);
        ...
        System.out.println("DEBUG AuthService: Checking if email exists...");
        // Check if email exists
        if (userDao.emailExists(email)) {
            throw new IllegalArgumentException("Email already registered");
        }

        System.out.println("DEBUG AuthService: Hashing password...");
        // Create user with hashed password
        String hashedPassword = PasswordUtil.hashPassword(password);
        User user = new User(email, hashedPassword);

        System.out.println("DEBUG AuthService: Creating user in DB...");
        user = userDao.createUser(user);

        System.out.println("DEBUG AuthService: Creating profile...");
        // Create profile
        Profile profile = new Profile(user.getId(), name);
        profileDao.createProfile(profile);

        System.out.println("DEBUG AuthService: Registration complete!");
        return user;
    }
